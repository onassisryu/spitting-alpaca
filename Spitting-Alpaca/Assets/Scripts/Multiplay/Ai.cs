...
public class Ai : LivingEntity
{
    ...
    private bool movementControlledExternally = false;
    ...
    void Update()
    {
        if (!isLive || movementControlledExternally)
        {
            isMoving = false;
        ...
        {
            animator.SetBool("isSpinned", true);  // 'Swing' 애니메이션 활성화
        }
        else
        {
            animator.SetBool("isSpinned", false);  // 높이가 낮아지면 'Swing' 애니메이션 비활성화
        }

    }

    public void UpdateMovementState(bool isActive)
    {
        movementControlledExternally = !isActive;
    }

    void StopMovement()
    {
        // Rigidbody의 모든 운동을 즉시 중지
        rb.velocity = Vector3.zero; // 속도를 0으로 설정
        rb.angularVelocity = Vector3.zero; // 회전 속도를 0으로 설정

        // 움직임을 멈추고 애니메이션 상태를 조정
        isMoving = false;
        animator.SetBool("isWalking", false);
        animator.SetBool("isRunning", false);
    ...
    IEnumerator DelayedDirectionChange()
    {
        ...
        // 다음 방향 변경까지의 시간 설정
        nextDirectionChangeTime = Time.time + Random.Range(0f, 5f);
    }
    ...
    public override async void Die()
    {
        ...
        StartCoroutine(DieCoroutie());
    }

    private IEnumerator DieCoroutie()
    {
